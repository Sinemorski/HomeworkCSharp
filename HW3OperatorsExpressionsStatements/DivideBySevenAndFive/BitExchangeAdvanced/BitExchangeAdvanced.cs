...
namespace BitExchangeAdvanced
{
    class BitExchangeAdvanced
    {
        static void Main(string[] args)
        {
            ...
            if ((p <= q && p + k-1 >= q) || (q <= p && q + k-1 >= p))
            {
                Console.WriteLine("overlapping");
                return;
            }
            Console.WriteLine("Двоично преставяне на въведеното число е: " + Convert.ToString(n, 2).PadLeft(32, '0'));
            for (int i = 1; i <= k; i++)
            {
            ...
            Console.WriteLine("Двоично преставяне на новото число е: " + Convert.ToString(n, 2).PadLeft(32, '0'));
